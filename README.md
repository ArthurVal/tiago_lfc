...
Generates tiago's `robot_description` using pre-determined files coming from the
dependencies:
- `tiago_description/robots/tiago.urdf.xacro`;
- `tiago_description/config/tiago_configuration.yaml`'s files to setup launch
  arguments based on the expected tiago's xacro mappings;

It can be used to create the URDF file generated by using the optional
`output_file:=<path>` launch argument.

#### `tiago_robot_state_publisher.launch.py`

Starts a `robot_state_publisher` using the tiago's `robot_desription` using the
same pre-determined files from `tiago_robot_description.launch.py`.

#### `tiago_sim.launch.py`

Firstly, starts tiago's `robot_state_publisher` (as if using
`tiago_robot_state_publisher.launch.py`).<br>
Launches a gazebo server with a given `world` (default to `empty.sdf`).<br>
Lastly, spawns the tiago's model inside the gazebo's world, with its controller
manager/model derived from `tiago_description`'s definition.
